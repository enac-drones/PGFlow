...
class Cases:
    def __init__(self, case_no=-1, arenamap=None, generate="manual", custom=None):
        ...
        if generate == "manual":
            ...
            if isinstance(custom, str):
                ...
                Vehicle2.Set_Goal([-3, 0, 0.5], 5, 0.000)
                # Vehicle3.Set_Goal([ 0,   3, 0.5], 5, 0.0001)
                ...
                Vehicle1.Set_Position([-3, 0.000, 0.5])
                Vehicle2.Set_Position([3, 0, 0.5])
                # Vehicle3.Set_Position([0,  -3 , 0.5])
                self.Vehicle_list = [
                    Vehicle1,
                ...
                Vehicle1.Set_Goal(
                    [3, 0.5, 0.5], 5, 0
                )  # goal,goal_strength 30, safety 0.001
                ...
                Vehicle1.Set_Goal(
                    [3, 2, 0.5], 5, 0
                )  # goal,goal_strength 30, safety 0.001 for vortex method
                ...
                Vehicle1.Set_Goal(
                    [3, 2, 0.5], 5, 0, 0
                )  # goal,goal_strength 30, safety 0.001, Vinfmag=0.5,0.5,1.5 for vortex method
                ...
                Vehicle1 = Vehicle(
                    "V1", 0, 0.85
                )  # Vehicle ID, Source_strength, im 0.85 veh 0.95
                ...
                Vehicle_list = [
                    Vehicle2
                ]  # , Vehicle2, Vehicle3] # , Vehicle2, Vehicle3] Vehicle1,Vehicle2,
                ...
                Vehicle1 = Vehicle(
                    "V1", 0, 0.85
                )  # Vehicle ID, Source_strength imaginary source = 0.75
        ...
        for (
            vehicle
        ) in self.Vehicle_list:  # FIXME Veh and veh ... select a norm for naming... :(
    ...
    def SetRandomStartGoal(self, arenamap, ID):
        ...
        while loop == True:
            ...
            d = (
                (goal_temp[0] - start_temp[0]) ** 2
                + (goal_temp[1] - start_temp[1]) ** 2
            ) ** 0.5
            if d > 1:
                for i in range(len(arenamap.buildings)):
                    ...
                    d_start = (
                        (x - start_temp[0]) ** 2 + (y - start_temp[1]) ** 2
                    ) ** 0.5
                    if d_goal < r * 2:
                        break
                    elif d_start < r * 2:
                    ...
                    if i == len(arenamap.buildings) - 1:
                        ...
                        loop = False
        vs = round(random.uniform(0, 0.5), 2)  # nosec
