...
def PolygonObstacle(sides,position, orientation, radius):
    print(f"PolygonObstacle has been called with {sides} sides, position {position}, angle {orientation}, radius {radius}")
    return True
...
def is_rotation(token):
    rotation_keywords = set(['rotate', 'rotation', 'rotational', 'rotated', 'turn', 'turned', 'turning',
                             'tilt','incline','inclined' 'tilted', 'tilting', 'angle', 'angled', 'angling', 'orient', 'orientation', 'oriented','spin'])
...
def get_obstacle_from_text(input_text):
    ...
    doc1 = nlp(input_text)
    doc = [token for token in doc1 if not token.is_stop and token.text.strip() not in stop_words]
    #print(doc)
    ...
    for index,token in enumerate(doc):
        ...
        if token.text.lower() in {"square", "triangle", "pentagon", "hexagon", "heptagon", "octagon"}:
            shape = token.text.lower()
        elif token.text.lower() == "radius" and index < len(doc) - 1 and doc[index+1].like_num:
           # print("inside size")
            radius = float(doc[index + 1].text)
        elif is_position(token) and index < len(doc) - 1 and is_coordinate(doc[index + 1]):
        ...
        elif token.text.lower() == "sides" and index < len(doc) - 1 and doc[index + 1].like_num:
            sides = int(doc[index + 1].text)
        elif token.like_num and index > 0 and index < len(doc) - 1 and doc[index + 1].text.lower() == "sides":
    ...
    if sides is None:
        shape_dict = {'square': 4, 'triangle': 3, 'pentagon': 5, 'hexagon': 6, 'heptagon': 7, 'octagon': 8}
    ...
    while True:
        ...
        if user_input == "exit":
            ...
            try:
                ...
                print("Error: input must be the the number of the building you wish to remove")
...
def create_vehicles():
    ...
    position = (0,0,0)
    while True:
        user_input = input(f"Specify the position of vehicle {i} in the format (x,y,z=1.2): ")
        if user_input == "none":
            return []
        else:
            try:
                ...
                if isinstance(tup, tuple):
                    ...
                    if isinstance(tup,tuple) and len(tup)==3 and all(type(n) == float or type(n) == int  for n in tup):
                        position = tup
                    break
                else:
