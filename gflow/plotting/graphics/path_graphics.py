...
class PathPatch:
    ...
    def create_patch(self, ax: Axes) -> Line2D:
        ...
        self.line, = ax.plot(x_coords, y_coords, color=self.line_color, linewidth=self.line_width)
        return self.line
...
class PathPlotter:
    ...
    def __init__(self, vehicle_data: List[dict]):
        self.paths = [PathEntity([tuple(p[:2]) for p in vehicle['path']]) for vehicle in vehicle_data]
