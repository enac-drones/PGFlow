...
class PanelFlow:
    ...
    def calculate_induced_sink_velocity(self, vehicle: Vehicle):
        ...
        induced_v = (
            -vehicle.sink_strength * position_diff_2D / (2 * np.pi * squared_distance)
        )
    ...
    def distance_effect_function(
        self, distance_array: np.ndarray, max_distance: float
    ) -> np.ndarray:
        ...
        effect[far_indices] = (
            1 / (2 * np.pi * distance_array[far_indices] ** 2)
        ) * exponential_dropoff[far_indices]

        return effect

    def calculate_induced_source_velocities_on_main_vehicle(
        ...
        other_source_strengths = np.array(
            [v.source_strength for v in other_vehicles.values()]
        )
        ...
        effect = self.distance_effect_function(
            distances, main_vehicle.max_avoidance_distance
        )

        # Calculate induced velocity
        induced_v = (
            other_source_strengths[:, np.newaxis]
            * position_diff_2D
            * effect[:, np.newaxis]
        )
    ...
    def calculate_induced_building_velocity(self, main_vehicle: Vehicle):
        ...
        for i, building in enumerate(buildings):
            ...
            if main_vehicle.ID in building.gammas:
                all_gammas[i, :num_panels] = building.gammas[main_vehicle.ID][
                    :num_panels
                ].ravel()
        ...
        # uv calculations
        uv = (
            all_gammas_normalized[:, :, np.newaxis]
            * vec_to_vehicle
    ...
    def gamma_calc(self, building: Building, vehicle: Vehicle):
        ...
        othervehicles = [
            v for v in othervehicles if not building.contains_point(v.position[:2])
        ]
        ...
        vel_sink = (
            ...
            * sink_diff
            / (2 * np.pi * sink_sq_dist)[:, np.newaxis]
        )
        vel_source_imag = (
            vehicle.imag_source_strength
            * imag_diff
            / (2 * np.pi * imag_sq_dist)[:, np.newaxis]
        )
        ...
        if othervehicles:
            ...
            all_source_strengths = np.array(
                [other.source_strength for other in othervehicles]
            )

            # Compute velocities due to sources and vortices
            effect = self.distance_effect_function(
                distances, vehicle.max_avoidance_distance
            )
            alternative = (
                all_source_strengths[:, np.newaxis]
                * source_diff
                * effect[..., np.newaxis]
            )
            vel_source_contribs = (
                all_source_strengths[:, np.newaxis]
                * source_diff
                / (2 * np.pi * source_sq_dist[..., np.newaxis])
            )
    ...
    def is_inside_buildings(
        self, buildings: list[Building], position: ArrayLike
    ) -> None | Building:
        for b in buildings:
    ...
    def Flow_Velocity_Calculation(self, vehicle: Vehicle) -> ArrayLike:
        vehicles = vehicle.personal_vehicle_dict

        V_gamma, V_sink, V_source, V_vortex, V_sum = [np.zeros(2) for _ in range(5)]
        ...
        if vehicle.relevant_obstacles:
            ...
            containing_building = self.is_inside_buildings(
                vehicle.relevant_obstacles, vehicle.position[:2]
            )
