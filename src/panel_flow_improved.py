...
class PanelFlow:
    def __init__(self,vehicles,arenamap,method = 'Vortex', update_velocities = True) -> None:
        self.vehicles = vehicles
        self.arenamap = arenamap
        self.method = method
        self.update_velocities = True
    ...
    def building_filter(self):
        ...
        for f,vehicle in enumerate(self.vehicles):
            ...
            othervehicleslist = self.vehicles[:f] + self.vehicles[f+1:]
            # othervehicleslist = []

            #print("a;lsdjfal;ksdjfl;askdjakl;sfjdls;jfa;lsdfj;lk")
            # Remove buildings with heights below cruise altitue:
            vehicle.altitude_mask = np.zeros(( len(self.arenamap.buildings) )) #, dtype=int)
            for index,panelledbuilding in enumerate(self.arenamap.buildings):
            ...
            related_buildings = list(compress(self.arenamap.buildings,vehicle.altitude_mask))

            # Vortex strength calculation (related to panels of each building):
            for building in related_buildings:
                building.gamma_calc(vehicle,othervehicleslist,self.arenamap,method = self.method)

            return None
        
    def vortex_method_calculation(self,f,vehicle):
        for building in self.arenamap.buildings:
